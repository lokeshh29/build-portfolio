...
export default function Contact() {
  ...
  return (
    <section id="contact" className="py-24 border-t border-slate-200 dark:border-slate-800/80 relative">
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-sky-400/10 dark:bg-sky-500/10 rounded-full blur-[140px] pointer-events-none -z-10" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h2 className="mt-2 text-3xl sm:text-5xl font-bold text-slate-900 dark:text-white tracking-tight">
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            {opportunityAreas.map((area) => (
              <span
                key={area}
                className="px-3 py-1 rounded-full text-xs font-medium bg-slate-100 dark:bg-slate-900/90 text-sky-700 dark:text-sky-300 border border-slate-200 dark:border-slate-800 shadow-sm"
              >
                {area}
              </span>
            ))}
