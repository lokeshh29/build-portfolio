...
export default function ProjectCard({
  ...
  return (
    <div className="glass-card rounded-2xl p-6 sm:p-8 flex flex-col justify-between transition-all duration-300 hover:border-sky-500/50 hover:-translate-y-1 group">
      <div>
        {/* Top Badges */}
        <div className="flex items-center justify-between gap-2 mb-4">
          <span className="px-2.5 py-0.5 rounded text-[11px] font-bold uppercase tracking-wider bg-sky-50 dark:bg-sky-500/10 text-sky-700 dark:text-sky-400 border border-sky-200 dark:border-sky-500/30">
            {project.badge}
          </span>
          {project.featured && (
        ...
        {/* Title & Tagline */}
        <h3 className="text-2xl font-bold text-slate-900 dark:text-white group-hover:text-sky-600 dark:group-hover:text-sky-300 transition-colors">
          {project.title}
        </h3>
        ...
        <div className="mt-6 pt-5 border-t border-slate-200 dark:border-slate-800/80">
          ...
          <ul className="space-y-2">
            {project.engineeringHighlights.map((highlight, idx) => (
              ...
              >
                <CheckCircle className="h-4 w-4 text-sky-600 dark:text-sky-400 shrink-0 mt-0.5" />
                <span>{highlight}</span>
              </li>
            ))}
          </ul>
      ...
      <div className="mt-8 pt-5 border-t border-slate-200 dark:border-slate-800/80">
        ...
        <div className="flex flex-wrap gap-1.5 mb-5">
          ...
          {project.technologies.length > 7 && (
            <span className="px-2 py-0.5 rounded text-[11px] font-medium bg-slate-100 dark:bg-slate-900 text-slate-500 border border-slate-200 dark:border-slate-800">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex-1 inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-xl text-xs font-semibold text-sky-700 dark:text-sky-300 bg-sky-50 hover:bg-sky-100 dark:bg-sky-500/10 dark:hover:bg-sky-500/20 border border-sky-300 hover:border-sky-400 dark:border-sky-500/30 dark:hover:border-sky-400 transition-all shadow-sm"
          >
