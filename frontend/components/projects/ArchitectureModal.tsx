...
import { X, Network, CheckCircle2, ArrowRight } from "lucide-react";
...
export default function ArchitectureModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-10">
      ...
      {/* Modal Card */}
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-[#0D111A] border border-slate-700/80 rounded-2xl shadow-2xl shadow-black/80 p-6 sm:p-8 z-10 text-slate-100">
        {/* Header */}
          ...
          <div>
            ...
            <div className="space-y-3">
              {data.flowSteps.map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-4 p-3.5 rounded-xl bg-slate-900/90 border border-slate-800/80"
                >
                  <div className="flex items-center justify-center h-7 w-7 rounded-lg bg-sky-500/10 text-sky-400 font-mono text-xs font-bold shrink-0">
                    0{idx + 1}
                  </div>
                  ...
                  {idx < data.flowSteps.length - 1 && (
                    <ArrowRight className="h-4 w-4 text-slate-600 shrink-0 self-center hidden sm:block" />
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {data.components.map((comp, idx) => (
                ...
                >
                  ...
                  <span className="text-slate-400">{comp.role}</span>
                </div>
              ))}
            </div>
          </div>
